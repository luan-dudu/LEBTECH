    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-slate-700/50 border border-slate-600 rounded-xl p-6">
          <h3 className="text-lg font-semibold text-white mb-4">{editingId ? 'Editar Usuário' : 'Criar Novo Usuário'}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <input type="text" placeholder="Nome completo" value={formData.name}
                ...
                users.map((user) => (
                  <tr key={user.id} className="border-b border-slate-700/50 hover:bg-slate-700/20 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-400">
                      {user.lastSignedIn ? new Date(user.lastSignedIn).toLocaleString('pt-BR') : 'Nunca'}
                    </td>
                    <td className="px-6 py-4 text-sm">
                      <div className="flex gap-2">
