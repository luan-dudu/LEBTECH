...
export default function ConsultingModal({ isOpen, onClose }: ConsultingModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', email: '', phone: '', company: '', consultingType: 'strategy', currentChallenges: '', objectives: '', teamSize: '', budget: '' });
      onClose();
    } catch (error: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-card border border-border rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label htmlFor="consultingType" className="block text-sm font-semibold text-foreground">Tipo de Consultoria *</label>
              <select id="consultingType" name="consultingType" value={formData.consultingType} onChange={handleChange}
            ...
            <div className="space-y-2">
              <label htmlFor="teamSize" className="block text-sm font-semibold text-foreground">Tamanho do Time de TI</label>
          ...
          <div className="space-y-2">
            <label htmlFor="currentChallenges" className="block text-sm font-semibold text-foreground">Desafios Atuais *</label>
            <textarea id="currentChallenges" name="currentChallenges" value={formData.currentChallenges} onChange={handleChange} required rows={4}
          ...
          <div className="space-y-2">
            <label htmlFor="objectives" className="block text-sm font-semibold text-foreground">Objetivos Esperados *</label>
            <textarea id="objectives" name="objectives" value={formData.objectives} onChange={handleChange} required rows={4}
              className="w-full px-4 py-3 rounded-lg bg-background border border-border focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all text-foreground placeholder-foreground/40 resize-none"
              placeholder="Quais são seus objetivos com a consultoria?" />
          </div>
          <div className="space-y-2">
            <label htmlFor="budget" className="block text-sm font-semibold text-foreground">Orçamento Disponível</label>
